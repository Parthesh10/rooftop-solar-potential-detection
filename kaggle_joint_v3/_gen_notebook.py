...
Round 3: Inria plus **1325 tiles from 25 AOIs on four continents**.

This is a single-variable experiment and the variable is *breadth*. The current
default (joint v2) trained on 505 tiles from ten Indian cities at ~35% of each
epoch, and beat its predecessor on held-out blocks in six of six new cities
while costing 0.0022 pooled Inria IoU. But v2 moved two things at once — the
tile count and the epoch share — so which one mattered was never established.

Here the share is held at 34.8% (1325 tiles x --extra-repeat 3, against v2's
505 x 8 = 35.2%) and only the places change: eight Indian tier-2 cities, four
African, three Southeast Asian and two Latin American. If the held-out numbers
move, breadth is the lever and the claim generalises past India. If they do not,
this is the ceiling that fact 36 asked someone to go and find.

Labels come from Google Open Buildings above its own **per-region** precision
threshold — which ranges from 0.651 in Lagos to 0.781 in Jakarta, so a single
global cutoff would be wrong in both directions — unioned with OpenStreetMap
where Overpass answered. Detections below the threshold become *ignore* rather
than background: confidence tracks building size, and teaching "small roof =
background" is the exact failure this data exists to fix.

There is no Inria-only control in the sweep on purpose: the 2026-09-03 model
...
INDIAN_SLUG = "partheshgupta/rooftop-solar-india-wide-tiles"
...
with io.open("kaggle_joint_v3/kaggle_joint_v3_train.ipynb", "w", encoding="utf-8") as fh:
