...
# Round 3. Two things change from the run that produced joint_v3:
#
#   1. The Indian half is 505 tiles from TEN cities (Delhi, Mumbai, Chennai,
#      Kolkata, Hyderabad, Jaipur, Ahmedabad, Pune, plus the original Bangalore
#      and Bhopal) instead of 103 from two, labelled from Google Open Buildings
#      above its own per-region precision threshold, unioned with OSM.
#
#   2. The share of each epoch that is Indian goes UP.
#
# Why (2). Measured 2026-09-06, joint training at ~22% Indian kept Inria almost
# intact (0.7662 against the shipped 0.7712, where fine-tuning costs ~0.10) but
# reached only 0.390 Bangalore footprint recall where fine-tuning on the *same*
# tiles reached 0.716, and left the silent fraction at 0.560 - exactly the
# shipped model's. That is under-learning, not a data-quantity problem, and the
# weighting was the one variable never tested.
#
# Inria contributes 155 x 48 = ~7.4k windows an epoch. 505 tiles at --extra-repeat
# 8 is ~4.0k, i.e. ~35% of each epoch against the previous 22%. Repeat 8 rather
# than 20 because there are now 4.9x more distinct tiles: the same share is
# reached with far less repetition, so the model sees variety instead of the same
# hundred tiles over and over.
SWEEP = [
    ("J2_joint_india10", ["--arch", "unet++", "--encoder", "efficientnet-b0",
                          "--samples-per-tile", "48", "--epochs", "50",
                          "--extra-data-dir", str(EXTRA),
                          "--extra-ignore-value", "128",
                          "--extra-repeat", "8"],
     "U-Net++ / EfficientNet-B0 on Inria + 505 tiles from ten Indian cities"),
]
...
with io.open("kaggle_joint_v2/kaggle_joint_v2_train.ipynb", "w", encoding="utf-8") as fh:
